...
!function (window) {
    ...
    CitySelect.prototype.open = function () {
        ...
        if (defaultSet.tj) {
            _this.loadTj();
            _this.ForwardView(false);
            _this.setNavTxt(3, defaultSet.tj);
        }
...
//		    }
    ...
    CitySelect.prototype.loadProvance = function () {
        ...
        $.each(_this.citys, function (k, v) {
            arr.push($('<a class="' + (v.name == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span>' + v.name + '</span></a>').data({
                citys: v.provance,
                tag: 0
            }));
        });
    ...
    CitySelect.prototype.loadCity = function () {
        ...
        $.each(cityData, function (k, v) {
            arr.push($('<a class="' + (v.name == _this.defaultSet.city ? 'crt' : '') + '" href="javascript:;"><span>' + v.name + '</span></a>').data({
                citys: v.city,
                tag: 1
            }));
        });
    ...
    CitySelect.prototype.loadArea = function () {
        ...
        $.each(areaData, function (k, v) {
            arr.push($('<a class="' + (v.name == _this.defaultSet.area ? 'crt' : '') + '" href="javascript:;"><span>' + v.name + '</span></a>').data({
            	citys: v.area,
            	tag: 2
            }));
        });
    ...
    CitySelect.prototype.loadTj = function () {
        var _this = this;

        var tjData = _this.$itemBox.eq(1).find('a.crt').data('citys');

        var arr = [];
        $.each(tjData, function (k, v) {
            arr.push($('<a class="' + (v.name == _this.defaultSet.tj ? 'crt' : '') + '" href="javascript:;"><span>' + v.name + '</span></a>').data({
            	citys: v.tj,
            	tag: 3
            }));
        });
