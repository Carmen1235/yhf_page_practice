    ...
    MobileCalendar.prototype = {
        ...
        bindEvent: function(type) {
            ...
            function popupDate(e) {
                _self.value=changeFormForIOS(_self.value)
            ...
            function dateCtrlInit() {
                var date = _self.value ? new Date(_self.value) : new Date();
                var dateArr = { yy: date.getYear(), mm: date.getMonth(), dd: date.getDate() - 1 };
                if (/^\d{4}[-\/]\d{1,2}[-\/]\d{1,2}$/.test(_self.value)) {
                    var rs = _self.value.match(/(^|[-\/])\d{1,4}/g);
                    dateArr.yy = rs[0] - _self.minY;
                    dateArr.mm = rs[1].replace(/[-\/]/g, "") - 1;
                    dateArr.dd = rs[2].replace(/[-\/]/g, "") - 1;
                } else { dateArr.yy = dateArr.yy + 1900 - _self.minY; }
            ...
            function ymCtrlInit() {
                var date = _self.value ? new Date(_self.value) : new Date();
                var dateArr = { yy: date.getYear(), mm: date.getMonth() };
                if (/^\d{4}[-\/]\d{1,2}$/.test(_self.value)) {
                    var rs = _self.value.match(/(^|[-\/])\d{1,4}/g);
                    dateArr.yy = rs[0] - _self.minY;
                    dateArr.mm = rs[1].replace(/[-\/]/g, "") - 1;
                } else { dateArr.yy = dateArr.yy + 1900 - _self.minY; }
            ...
            function popupzdDateTime(e) {
                ...
                _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
                    ...
                    '<div class="datetime_roll">' +
                    '<div>' +
                    '<div class="gear date_dd" data-datetype="date_dd" style="text-indent:0;"></div>' +
                    '<div class="date_grid">' +
                    '</div>' +
                    '</div>' +
                    '<div>' +
                    '<div class="gear time_hh" data-datetype="time_hh" style="text-indent:0;"></div>' +
                    '<div class="date_grid">' +
                    '</div>' +
                    '</div>' +
                    '<div>' +
                    '<div class="gear time_mm" data-datetype="time_mm" style="text-indent:0;"></div>' +
                    '<div class="date_grid">' +
                    '</div>' +
                    '</div>' +
                    '</div>' +
                ...
                lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
                var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
                lcalendar_finish.addEventListener('touchstart', finishzdMobileDateTime);
                var lcalendar_bg = _self.gearDate.querySelector(".date_bg");
                lcalendar_bg.addEventListener('click', closeMobileCalendar);
                var date_dd = _self.gearDate.querySelector(".date_dd");
                var time_hh = _self.gearDate.querySelector(".time_hh");
                ...
                time_mm.addEventListener('touchend', gearTouchEnd);
                lcalendar_cancel.addEventListener('click', closeMobileCalendar);
                lcalendar_finish.addEventListener('click', finishzdMobileDateTime);
                date_dd.addEventListener('mousedown', gearTouchStart);
                time_hh.addEventListener('mousedown', gearTouchStart);
                time_mm.addEventListener('mousedown', gearTouchStart);
                date_dd.addEventListener('mousemove', gearTouchMove);
            ...
            function dateTimeCtrlInit() {
                var date = _self.value ? new Date(_self.value) : new Date(),
                    getMinute;
                if (date.getMinutes() > 30 || date.getMinutes() == 0) { getMinute = 0 } else { getMinute = 30 };
                var dateArr = { yy: date.getYear(), mm: date.getMonth(), dd: date.getDate() - 1, hh: date.getHours(), mi: getMinute };
                if (/^\d{4}[-\/]\d{1,2}[-\/]\d{1,2}\s\d{2}:\d{2}$/.test(_self.value)) {
                    var rs = _self.value.match(/(^|[-\/]|\s|:)\d{1,4}/g);
                    dateArr.yy = rs[0] - _self.minY;
                    dateArr.mm = rs[1].replace(/[-\/]/g, "") - 1;
                    dateArr.dd = rs[2].replace(/[-\/]/g, "") - 1;
                ...
                _self.gearDate.querySelector(".date_yy").setAttribute("val", dateArr.yy);
                _self.gearDate.querySelector(".date_mm").setAttribute("val", dateArr.mm);
                _self.gearDate.querySelector(".date_dd").setAttribute("yy", dateArr.yy);
                _self.gearDate.querySelector(".date_dd").setAttribute("mm", dateArr.mm);
                _self.gearDate.querySelector(".date_dd").setAttribute("val", dateArr.dd);
                setDateGearTooth();
            ...
            function timeCtrlInit() {
                var date = _self.value ? new Date(_self.value) : new Date(),
                    getMinute;
                if (date.getMinutes() > 30) { getMinute = 0 } else { getMinute = 1 };
                var e = { hh: d.getHours(), mm: getMinute };
                if (/^\d{2}:\d{2}$/.test(_self.value)) {
                    var rs = _self.value.match(/(^|:)\d{2}/g);
            ...
            function setDateGearTooth() {
                ...
                if (date_yy && date_yy.getAttribute("val")) {
                    ...
                    for (var p = 0; p <= passY - 1; p++) { itemStr += "<div class='tooth'>" + (_self.minY + p) + "</div>"; }
                ...
                if (date_mm && date_mm.getAttribute("val")) {
                    ...
                    var maxM = 11;
                    var minM = 0;
                    if (yyVal == passY - 1) { maxM = _self.maxM - 1; }
                ...
                if (date_dd && date_dd.getAttribute("val")) {
                    ...
                    if (yyVal == passY - 1 && _self.maxM == mmVal + 1) { maxD = _self.maxD - 1; }
            ...
            function setzdDateGearTooth() {
                var passY = _self.maxY - _self.minY + 1;
                var passY = 1;//判断不跨年.......
                var itemStr = "";
                var date_mm = _self.gearDate.querySelector(".date_dd");
                if (date_mm && date_mm.getAttribute("mm")) {
                    itemStr = "";
                    var mmVal = parseInt(date_mm.getAttribute("mm"));
                    var maxM = 11;
                    var minM = 0;
                    if (yyVal == passY - 1) { maxM = _self.maxM - 1; }
                    if (yyVal == 0) { minM = _self.minM - 1; }
                ...
                var date_dd = _self.gearDate.querySelector(".date_dd");
                var yyVal = date_dd.getAttribute("yy");
                var mmVal = date_dd.getAttribute("mm");
                ...
                if (date_dd && date_dd.getAttribute("val")) {
                    ...
                    var min_day = myDate.getDate();
                    if(mmVal > now_month){
                        mmVal = now_month;
                        ddVal = ddVal + calcDays(yyVal, mmVal);
                        date_dd.setAttribute("mm",mmVal);
                    }
                    
                    if(mmVal == 0 && now_month == 11){//判断是选中1月份的时候..........
                        mmVal = now_month;
                        ddVal = ddVal + calcDays(yyVal, mmVal);
                        date_dd.setAttribute("mm",mmVal);
                    }
                    ...
                    var maxD = maxMonthDays - 1;
                    var minD = 0;
                    yyVal = 0;//为了还原打开默认显示的位置664.....
                    if (yyVal == passY - 1 && _self.maxM == Number(mmVal) + 1) { maxD = _self.maxD - 1; }
                    if (yyVal == 0 && _self.minM == Number(mmVal) + 1) { minD = _self.minD - 1; }
                    var weekday = ['日', '一', '二', '三', '四', '五', '六'];
                    var toody = new Date();
                    mmVal = Number(mmVal)+1;
                    var nums = 0;
                    for (var p = min_day; p <= maxMonthDays; p++) {
                        if(nums < 14){
                            var date = new Date(_self.minY+"/"+ mmVal +"/"+(p));
                            var todata = mmVal+"月"+p+"日"+"周"+ weekday[date.getDay()];
                            if(p == min_day){   //判断当天日期
                                todata = "今天";
                            }
                            itemStr += "<div class='tooth'>" +todata+ "</div>";
                            nums++;
                        }
                    }
                    mmVal = Number(mmVal)+1;
                    var yearnum = 0;//............
                    if(mmVal == 13){mmVal = 1;yearnum = 1;}//判断月份等于13则.........
                    if(nums < 14){
                        var max_p = 14-nums;
                        for (var p = 1; p <= max_p; p++) {
                            if(nums < 14){
                                var date = new Date((_self.minY + yearnum) + "/"+ mmVal +"/"+(p));
                                var todata = mmVal + "月" + p + "日" + "周" + weekday[date.getDay()];
                                itemStr += "<div class='tooth'>" +todata+ "</div>";
                                nums++;
                            }
                    ...
                    if(mmVal == 1 && now_month == 11){mmVal = 13;}//判断月份等于13则............
                    date_dd.innerHTML = itemStr;
                    date_dd.style["-webkit-transform"] = 'translate3d(0,' + (8 - (ddVal - minD) * 2) + 'em,0)';
                    date_dd.setAttribute('top', 8 - (ddVal - minD) * 2 + 'em');
                    //还原位置。。。。。。。
                    _self.gearDate.querySelector(".date_dd").setAttribute("val", ddVal);
                    //还原位置。。。。。。。
                } else { return; }
            ...
            function setTimeGearTooth() {
                ...
                if (time_hh && time_hh.getAttribute("val")) {
                    ...
                    for (var g = 8; g <= 18; g++) { i += "<div class='tooth'>" + g + "</div>"; }
                    time_hh.innerHTML = i;
                    time_hh.style["-webkit-transform"] = 'translate3d(0,' + (24 - hhVal * 2) + 'em,0)';
                    time_hh.setAttribute('top', 24 - hhVal * 2 + 'em');
                } else { return }
                var time_mm = _self.gearDate.querySelector(".time_mm");
                if (time_mm && time_mm.getAttribute("val")) {
                    var i = "";
                    var mmVal = parseInt(time_mm.getAttribute("val"));
                    for (var g = 0; g <= 1; g++) { i = "<div class='tooth'>" + 0 + "</div><div class='tooth'>" + 30 + "</div>"; }
                    time_mm.innerHTML = i;
                    if (mmVal == 30) { mmVal = 1 }
            ...
            function gearTouchStart(e) {
                ...
                e.preventDefault();
                var target = e.target;
                while (true) { if (!target.classList.contains("gear")) { target = target.parentElement; } else { break } }
                clearInterval(target["int_" + target.id]);
                target["old_" + target.id] = e.targetTouches ? e.targetTouches[0].screenY : e.pageY;
                target["o_t_" + target.id] = (new Date()).getTime();
                var top = target.getAttribute('top');
                if (top) { target["o_d_" + target.id] = parseFloat(top.replace(/em/g, "")); } else { target["o_d_" + target.id] = 0; };
                pree = e;
            }
            ...
            function gearTouchEnd(e) {
                ...
                if (Math.abs(flag) <= 0.2) { target["spd_" + target.id] = (flag < 0 ? -0.08 : 0.08); } else { if (Math.abs(flag) <= 0.5) { target["spd_" + target.id] = (flag < 0 ? -0.16 : 0.16); } else { target["spd_" + target.id] = flag / 2; } }
                if (!target["pos_" + target.id]) { target["pos_" + target.id] = 0; }
                rollGear(target);
                pree = null;
            };

            function gearTouchOut(e) { gearTouchEnd(pree); };
            ...
            function rollGear(target) {
                ...
                target["int_" + target.id] = setInterval(function() {
                    ...
                    pos += speed;
                    if (Math.abs(speed) > 0.1) {} else { speed = 0.1; var b = Math.round(pos / 2) * 2; if (Math.abs(pos - b) < 0.05) { stopGear = true; } else { if (pos > b) { pos -= speed } else { pos += speed } } }
                    if (pos > 8) {
                        pos = 8;
                        stopGear = true;
                    }
                    switch (target.dataset.datetype) {
                        ...
                        case "date_mm":
                            ...
                            break;
                        case "date_dd":
                            var date_yy = _self.gearDate.querySelector(".date_dd");
                            var date_mm = _self.gearDate.querySelector(".date_dd");
                            var yyVal = parseInt(date_yy.getAttribute("yy"));
                            var mmVal = parseInt(date_mm.getAttribute("mm"));
                            var maxMonthDays = calcDays(yyVal, mmVal);
                            var maxD = maxMonthDays - 1;
                            var minD = 0;
                            ...
                            var top_num = 0;
                            if(maxD - minD < 14){
                                top_num = 14 - (maxD - minD);
                            }
                            // 自定义，增加滑动距离
                            var minTop = 8 - (maxD - minD + top_num - 1 ) * 2;
                            if (pos < minTop) {
                                pos = minTop;
                                stopGear = true;
                            }
            ...
            function setGear(target, val) {
                ...
                if (/date/.test(target.dataset.datetype)) { setDateGearTooth(); } else { setTimeGearTooth(); }
            }
            ...
            function finishzdMobileDateTime(e) {
                var passY = _self.maxY - _self.minY + 1;
                passY = 1 ;//判断不跨年.......
                var date_yy = parseInt(Math.round(_self.gearDate.querySelector(".date_dd").getAttribute("yy")));
                var date_mm = parseInt(Math.round(_self.gearDate.querySelector(".date_dd").getAttribute("mm"))) + 1;
                var date_dd = parseInt(Math.round(_self.gearDate.querySelector(".date_dd").getAttribute("val"))) + 1;
                var maxMonthDays = calcDays(date_yy, date_mm -1);
                var yearnum = 0;//.........
                //cun_dd = date_dd;
                if(date_dd > maxMonthDays){
                    date_dd = date_dd - maxMonthDays;
                    date_mm = date_mm + 1;
                }
                if(date_mm == 13){date_mm = 1;yearnum = 1;}//判断月份等于13则.........
                var date = new Date((date_yy % passY + _self.minY + yearnum) + "-" + date_mm + "-" + date_dd);
                var select_week = date.getDay();
                if(select_week == 0 || select_week == 6){
                    YDUI.dialog.toast('休息日无法安排面试，请重新选择', 'none', 1000);
                    return false;
                }
                ...
                if (time_mm == 1) { time_mm = 30 }
                time_mm = time_mm > 9 ? time_mm : '0' + time_mm;
                _self.trigger.value = (date_yy % passY + _self.minY + yearnum) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
                _self.value = _self.trigger.value;
                if (_self.onSubmit) _self.onSubmit();
                closeMobileCalendar(e);
            }
            ...
            function changeFormForIOS(v){ //支持ios
                return (v || "").replace(/-/g, "/");
            }
            _self.trigger.addEventListener('click', { "ym": popupYM, "date": popupDate, "datetime": popupDateTime,"zdtime": popupzdDateTime, "time": popupTime }[type]);
        }
    }
