...
! function (window) {
  ...
  CitySelect.prototype.open = function () {
    ...
    if(_this.defaultSet.provance === ''){
      $('.m-cityselect .cityselect-header .cityselect-nav a').eq(1).html('');
    }
    if(_this.defaultSet.city === ''){
      $('.m-cityselect .cityselect-header .cityselect-nav a').eq(2).html('');
    }
    if(_this.defaultSet.area === ''){
      //$('.cityselect-content.cityselect-move-animate').removeClass('cityselect-next-two');
      //$('.cityselect-content.cityselect-move-animate').addClass('cityselect-next-three');
      $('.cityselect-content.cityselect-move-animate').removeClass('cityselect-next-two');
      $('.cityselect-content.cityselect-move-animate').addClass('cityselect-next');
      $('.m-cityselect .cityselect-header .cityselect-nav a').eq(3).html('');
    }
  };
  ...
  CitySelect.prototype.createDOM = function () {
    ...
    _this.$cityElement = $('' +
      ...
      '        <p class="cityselect-title">请选择您的考试</p>' +
    ...
    _this.$cityElement.on('click.ydui.cityselect', '.cityselect-nav a', function () {
      $('.cityselect-content').removeClass('cityselect-next-three');
      var $this = $(this);
  ...
  CitySelect.prototype.setNavTxt = function (index, txt, id) {
    ...
    if(index >= 0){
      ...
      $nav.eq(index + 1).removeClass('crt').html('请选择');
      $nav.eq(index - 1).removeClass('crt');
  ...
  CitySelect.prototype.bindItemEvent = function () {
    ...
    $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
      ...
      var $this = $(this);
      if ($this.hasClass('crt')) return;//是否选中,是就不进行操作
      $this.addClass('crt').siblings().removeClass('crt');//除选中的 其他清除
      var tag = $this.data('tag');
      var id = $this.attr("cid");
      _this.setNavTxt(tag, $this.text(), id);//--吧数据放上去，去除除选中的所有状态
      var $nav = $cityElement.find('.cityselect-nav a'),
      defaultSet = _this.defaultSet;
        
      if (tag == 0) {
        if(_this.loadProvance()){
          ...
          defaultSet.id = $nav.eq(0).attr("cid");
          $this.addClass('crt');
          _this.returnValue();
        }
        ...
        if(_this.loadCity()){
          ...
          defaultSet.city = $nav.eq(2).html();
          defaultSet.area = $nav.eq(3).html();
          defaultSet.id = $nav.eq(0).attr("cid") + '-' + $nav.eq(1).attr("cid");
          _this.returnValue();
        }
        ...
        $('.cityselect-content.cityselect-move-animate').removeClass('cityselect-next-three');
        if(_this.loadArea()){
          defaultSet.country = $nav.eq(0).html();
          defaultSet.provance = $nav.eq(1).html();
          defaultSet.city = $nav.eq(2).html();
          defaultSet.area = $nav.eq(3).html();
          defaultSet.id = $nav.eq(0).attr("cid") + '-' + $nav.eq(1).attr("cid") + '-' + $nav.eq(2).attr("cid");
          _this.returnValue();
        ...
        _this.defaultSet.country = $nav.eq(0).html();
        _this.defaultSet.provance = $nav.eq(1).html();
        _this.defaultSet.city = $nav.eq(2).html();
        _this.defaultSet.area = $nav.eq(3).html();
        _this.defaultSet.id = $nav.eq(0).attr("cid") + '-' + $nav.eq(1).attr("cid") + '-' + $nav.eq(2).attr("cid") + '-' + $nav.eq(3).attr("cid");
       _this.returnValue();
      }
  ...
  CitySelect.prototype.returnValue = function () {
    var _this = this
    _this.defaultSet.country = _this.defaultSet.country === '请选择'? '' : _this.defaultSet.country;
    _this.defaultSet.provance = _this.defaultSet.provance === '请选择'? '' : _this.defaultSet.provance;
    _this.defaultSet.city = _this.defaultSet.city === '请选择'? '' : _this.defaultSet.city;
    _this.defaultSet.area = _this.defaultSet.area === '请选择'? '' : _this.defaultSet.area;
    var defaultSet = _this.defaultSet;
